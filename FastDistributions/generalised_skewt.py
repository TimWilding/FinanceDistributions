...
def var_adjustment_sgt(ƛ, pinv, q):
    ...
    a = (1+3*(ƛ**2)) *np.exp(a_adj) #* gamma(3*pinv) * gamma(q - 2*pinv) / gamma(pinv) / gamma(q)
...
def generalised_skewt_loglikelihood(x, μ, σ, ƛ, k, n):
    ...
    # This doesn't appear to be a good approx
        return np.log(uniform.pdf(x, μ - np.sqrt(3)*σ*(1-ƛ), 2*np.sqrt(3)*σ*(1+ƛ)))
...
class GeneralisedSkewT(rv_continuous):
    ...
    def _logpdf(self, x):
        return generalised_skewt_loglikelihood(x, self.μ, self.σ, self.ƛ, self.k, self.n)
    ...
    def _mean(self):
        ...
        lam_adj = self.ƛ * v * gamma(2*p_inv) * gamma(q-p_inv) / gamma(p_inv) / gamma(q)
