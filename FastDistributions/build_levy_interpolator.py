...
def process(i):
    ...
    for j in range(N_BETA):
        parallel_pdf_vals[i, j, :] = np.log(levy_stable(alpha_vals[i], beta_vals[j]).pdf(x_tan_vals))
...
test_vals= np.abs(parallel_pdf_vals[:, :, 2:] + parallel_pdf_vals[:, : , 0:N_PTS-3] - parallel_pdf_vals[:, : ,1:N_PTS-2])
